...
class ArticleModelTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.article = Article.objects.create(title = 'Article',
            ...
            outlet_id = self.outlets[2].id, author_id = self.authors[2].id)
    ...
    def test_article_check_author(self):
        """Tests if a newly created article's author is correct."""
        self.assertEqual(self.article.author, self.authors[2])
    ...
    def test_article_delete_author(self):
        """Tests if cascade deletion is working for authors."""
        save_all_articles = [article.save() for article in self.articles]
        old_count = Article.objects.count()
